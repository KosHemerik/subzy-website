...
export default function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    const initFromRecoveryLink = async () => {
      ...
      try {
        ...
        if (tokenHash && type) {
          const allowedTypes = ["recovery", "email"];
        ...
        if (isMounted) {
          setStatus("invalid");
          setError(err?.message || "Deze resetlink is ongeldig of verlopen.");
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Sign out so the owner is required to sign in fresh with the new password
  ...
  if (status === "checking") {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-gray-600">Resetlink controleren...</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 text-center">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">{error || "Deze resetlink is ongeldig of verlopen."}</p>
        </div>
        <Link href="/wachtwoord-vergeten" className="inline-flex items-center text-secondary hover:text-accent font-medium transition">
    ...
    return (
      <div className="space-y-6 text-center">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">Uw wachtwoord is gewijzigd. U wordt doorgestuurd naar de inlogpagina.</p>
        </div>
      </div>
    );
  }
